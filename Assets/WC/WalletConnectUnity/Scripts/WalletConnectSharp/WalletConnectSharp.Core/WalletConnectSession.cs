...
namespace WalletConnectSharp.Core
{
    public class WalletConnectSession : WalletConnectProtocol
    {
        ...
        public WalletConnectSession(SavedSession savedSession, bool usePinKey = false, ITransport transport = null, ICipher cipher = null, EventDelegator eventDelegator = null) : base(savedSession, transport, cipher, eventDelegator)
        {
            ...
            this.usePinKey = usePinKey;
        }

        public WalletConnectSession(ClientMeta clientMeta, bool usePinKey = false, string bridgeUrl = null, ITransport transport = null, ICipher cipher = null, int chainId = 1, EventDelegator eventDelegator = null) : base(transport, cipher, eventDelegator)
        {
            if (clientMeta == null)
            {
        ...
        private void GenerateKey()
        {
            ...
            if (this.usePinKey)
            {;
                int randomStartIndex = new Random().Next(0, 24);
                string stringPin = Guid.NewGuid().ToString("n").Substring(randomStartIndex, 6);
                
                var utf8 = new UTF8Encoding();
                byte[] secret = utf8.GetBytes(stringPin);

                // byte[] secret = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
                Debug.Log(BitConverter.ToString(secret));

                SHA256 sha256Hash = SHA256.Create();
                byte[] bytes = sha256Hash.ComputeHash(secret);
